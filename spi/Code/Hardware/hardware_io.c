...
int32_t HW_IO_SPI_send(uint8_t* pTxData, uint32_t Legnth)
{
	...
	if (HAL_SPI_Transmit(&cfg_spi_handler, pTxData, (uint16_t)Legnth, SPI_TIMEOUT) != HAL_OK)
	{
...
int32_t HW_IO_SPI_recv(uint8_t* pRxData, uint32_t Legnth)
{
	...
	if (HAL_SPI_Receive(&cfg_spi_handler, pRxData, (uint16_t)Legnth, SPI_TIMEOUT) != HAL_OK)
	{
		ret = HARDWARE_IO_ERROR;
	}
